...
%If the kernel function is \textit{positive-definite} then it is symmetric by definition of the inner product. 
...
In \cite{fukumizu2008kernel} and \cite{sriperumbudur2011universality}, a special class of kernel functions known as \textit{characteristic kernels} was introduced. The papers show that if $k$ is characteristic then the mapping of $\phi$ into the RKHS is unique. By combining the previous properties of kernel functions, we can now understand where the idea of the maximum mean discrepancy comes from. Through the kernel trick, $X$ and $Y$ are uniquely mapped to another feature space where the expected value of each is compared in this new feature space. 

Suppose $n$ samples from a set $X = \{x_1, x_2, ..., x_n\}$ and $m$ samples from a different set $Y=\{y_1, y_2, ..., y_m\}$ are derived from a metric space $\mathcal{X}$. Assume both are distributed as $X \sim  P$ and $Y \sim Q$ respectively. In \cite{smola2007hilbert}, the formal definition of the maximum mean discrepancy (MMD) is defined as:
\begin{equation}
\label{mmd_theory}
